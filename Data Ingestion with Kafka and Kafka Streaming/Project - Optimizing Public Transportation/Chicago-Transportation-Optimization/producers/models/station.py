...
class Station(Producer):
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        #
        #
        topic_name = f"com.udacity.cta.stations" # TODO: Come up with a better topic name
        super().__init__(
            topic_name,
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        print(f"{direction}")
        print(f"{self.color.name}")
        print(f"{train.status.name}")
        print(f"{prev_station_id}")
        print(f"{prev_direction}")
        
        print(self.value_schema)

        if prev_station_id == None:
            prev_station_id = 0
        if prev_direction == None:
            prev_direction = "NoneStr"
        print(f"{prev_station_id}")
        print(f"{prev_direction}")    
